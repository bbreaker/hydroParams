...
p2 <- ggplot() + 
  ...
  geom_line(data = testVals, aes(x = dates, y = muP, color = "mu"), size = 1) +
  #geom_line(data = testDFnon, aes(x = dates, y = nuP), color = "blue", size = 1) +
  geom_line(data = testVals, aes(x = dates, y = sigP, color = "sigma"), size = 1) +
...
p3 <- ggplot() + 
  geom_line(data = testVals, aes(x = dates, y = slope), color = "grey50", size = 1) +
  ...
  scale_y_continuous(limits = c(-0.01, 0.01)) +
  scale_x_datetime(limits = as.POSIXct(c("2017-03-01 00:00:00", "2017-06-01 00:00:00"), 
                                       origin = "1970-01-01 00:00:00", tz = "UTC")) +
  theme_bw()
